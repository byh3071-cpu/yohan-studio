...
## 네이버 발행 원커맨드 (2026-07-20)

```powershell
pnpm naver -- <slug>
```

`.md` 원고가 준비된 상태에서 [변환 → fragment 클립보드 적재 → 글쓰기 열기]까지 한 번에. 사람은 ①본문 Ctrl+V ②✍ 진솔 슬롯 채우기 ③제목·발행·태그 ④URL 회신, 4가지만.

- 이미지: `.md` 마커에 라이브 URL을 붙이면(`[이미지 삽입: 설명 | https://yohanstudio.co/...]`) **붙여넣기만으로 SE 이미지 컴포넌트로 자동 삽입**된다 (2026-07-20 에디터 실측). 수동 삽입 불필요.
