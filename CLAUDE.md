...
## 현재 상태 (마지막 갱신: 2026-06-15)
- **Phase**: 3 (Supabase + Stripe + 스토어 + 챗봇/TTS + 검색 — **코드완료, 라이브 배포 진행**)
- **완료**: ①~⑬ / store·checkout·webhook / showroom·diagnosis·services / AI 챗봇·TTS / Fuse 검색·AEO / `.env.example`
- **SEO 검증 (2026-06-15, 라이브 실측)**: robots.txt·sitemap.xml·canonical·OG·JSON-LD(+FAQPage)·GA(`G-P24R6S14MS`)·Google/Naver 인증 메타 모두 정상 작동. 인증값은 layout.tsx 하드코딩 폴백 존재(env 없어도 출력).
- **SEO 반복개선 (2026-06-15, P0~P3 적용)**: 홈 전용 메타·홈 OG 회귀(site_name/locale/twitter card) 수정 / `/services`·`/diagnosis` FAQPage+Breadcrumb / 공유 `ogCard` 헬퍼로 정적 7페이지 전용 OG 카드 / sitemap 정적 lastmod 안정화 / `next.config.ts`에 `*.vercel.app→yohanstudio.co` 308 canonical 강제. 빌드·lint·렌더 실측 통과.
- **진행 중**: Playwright QA / 배포 E2E 손검증 / 문서 정리
- **마지막 커밋**: `89fbafc feat(seo): per-page OG cards for blog/showroom/design/open-source via shared helper`
- **다음 작업**: ⚠️ **(사람)** Vercel `NEXT_PUBLIC_SITE_URL=https://yohanstudio.co` 설정 + 도메인 연결 확인 → 재배포 → canonical·OG 손검증
- **블로커**: 프로덕션 env `NEXT_PUBLIC_SITE_URL`이 vercel.app로 남으면 canonical이 308 리다이렉트와 충돌 — 위 #1 액션 필수
