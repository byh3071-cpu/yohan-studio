...
- **SEO 검증 (2026-06-15, 라이브 실측)**: robots.txt·sitemap.xml(19 URL)·canonical·OG·JSON-LD(+FAQPage)·GA(`G-P24R6S14MS`)·Google/Naver 인증 메타 모두 정상 작동. 인증값은 layout.tsx 하드코딩 폴백 존재(env 없어도 출력). `/portfolio`→`/showroom` 301 `next.config.ts`에 구현 확인.
- **진행 중**: Playwright QA / 배포 E2E 손검증 / 문서·dead code 정리 / SEO 반복개선(홈 OG·FAQ 스키마 확장)
- **마지막 커밋**: `391821f feat(seo): add site-wide default OG image for static routes`
- **다음 작업**: 재배포 후 OG 이미지 손검증 → SEO 반복개선 항목(아래) 적용
- **블로커**: (해소) Vercel env·Supabase 마이그레이션은 라이브 실측상 반영됨 — 잔여는 E2E 손검증뿐
