...
| MCP | 전송 | 용도 |
|---|---|---|
| playwright | stdio | 격리 Chromium UI 검증 — 클릭/스크린샷/콘솔 (현재 미연결, msedge 헤드리스로 임시 대체 중) |
...
| lazyweb | stdio | 디자인 레퍼런스 검색 / quick-references / 유사 화면 비교 (Windows에선 `.mcp.json`의 `command: "sh"` → `C:\\Program Files\\Git\\bin\\sh.exe` 풀패스 패치 필요) |

⚠️ MCP "Connected" ≠ "Authenticated". 첫 API 호출로 검증 필수.
⚠️ CLI 인자에 시크릿 금지. env-var 상속 패턴 사용.
⚠️ Edge `--headless=new --screenshot` 은 viewport 캡처. 풀페이지는 ~3000px 이상에서 실패 — Playwright 또는 puppeteer 같은 CDP 도구 필요.
...
| ⑪ | Hero 에디토리얼 보강 (Serif italic / ISSUE 라인 / eyebrow marquee) | ✅ 완료 |
| ⑫ | 모바일 헤더 햄버거 토글 (≤768px 드롭다운 패널 + ESC + 라우트 동기화) | ✅ 완료 |
| ⑬ | AEO/GEO 인프라 (/llms.txt + Person·Org·WebSite JSON-LD + FAQ + FAQPage schema) | ✅ 완료 |
...
## 현재 상태 (마지막 갱신: 2026-05-13 세션 4)

- **Phase**: 2 (블로그 + SEO + AEO) — 13/13 완료. Phase 3(Supabase + Stripe) 진입 준비 단계.
- **완료**: v2 디자인 / MDX 블로그 / SEO 파이프라인 / GA·GSC / 다크모드 / `/design` / BlogRowCard / Inter / 블로그 본문 / Hero 에디토리얼 / 모바일 햄버거 / AEO 인프라(llms.txt + JSON-LD @graph + FAQPage)
- **마지막 커밋**: `e3126e4 feat(seo): /llms.txt 추가 — AEO/GEO 답변 엔진 사이트 가이드`
- **다음 작업**:
  1. **FAQ Q&A 시안 검수** (백요한 톤/사실관계 통과) — `src/components/sections/Faq.tsx` 6개
  2. **답변 엔진 실측** — Perplexity / ChatGPT에서 "백요한 바이브코더", "Yohan Studio 1인 기업" 인용 여부 (인덱싱 며칠 소요)
  3. **모바일 헤더 클릭 동작 사람 검증** — 햄버거 드롭다운 펼침은 dev 서버(3050)에서 직접
  4. **Phase 3 시작 (5/17)** — ADR-001(결제 프로바이더), 사업자 등록 결정, SKU 1차 목록
- **블로커**:
  - 다크 모드 `--color-line: #334155` 가 surface `#141414` 위에서 콘트라스트 약함. CLAUDE.md 디자인 시스템 표는 `#F4F1EA` 명시 — 정합 결정 필요.
  - 개인정보 노출(`flexible-nine.vercel.app`) 더미 데이터 재배포 후 demo URL 복원 보류.
