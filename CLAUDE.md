...
## 현재 상태 (마지막 갱신: 2026-06-08)
- **Phase**: 3 (Supabase + Stripe + 스토어 + 챗봇/TTS + 검색 — **코드완료, 라이브 배포 블로커 있음**)
- **완료**: ①~⑬ / store·checkout·webhook / showroom·diagnosis·services / AI 챗봇·TTS / Fuse 검색·AEO / `.env.example`
- **진행 중**: portfolio→showroom 301 / Playwright QA / 배포 E2E / Home Featured 배치 / 문서·dead code 정리
- **마지막 커밋**: `4dd0170 chore(env): add .env.example template and allow git tracking`
- **다음 작업**: Vercel env 등록 + Supabase 마이그레이션(사람) → 재배포 → E2E 손검증
- **블로커**: Vercel 환경변수 미세팅 · Supabase 마이그레이션 대시보드 미실행 · `/portfolio` 레거시 라우트 잔존
